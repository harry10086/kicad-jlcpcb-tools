...
## 版本更新
* `v3.9.3`：分配的 LCSC 编号，默认在 Fab 层，默认不显示。修复之前 LCSC Params 字段一直显示在 F.Fab 层的问题。
* `v3.9.2`：设置增加选择：分配立创编号时，默认搜索 LCSC 编号，可选择搜索 Value 关键词。
* `v3.9.1`：当元件已有立创编号时，分配时搜索优先编号，无编号时搜索值。修复移除立创编号后再分配编号时不显示，需要分配两次的问题。调整编号列宽，优先显示全部编号。
* `v3.9.0`：分配立创编号后，元件属性里添加的 LCSC Params 字段修改为默认不显示，并且在 Fab 层。
