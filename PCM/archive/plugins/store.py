...
class Store:
    ...
    def create_db(self):
        ...
        with contextlib.closing(sqlite3.connect(self.dbfile)) as con, con as cur:
            ...
            cur.execute(
                "CREATE TABLE IF NOT EXISTS part_details_cache ("
    ...
    def save_part_details_cache(self, lcsc: str, stock: Union[int, None], type: str, description: str):
        """Cache the part details to avoid blanking out the UI on reload."""
        with contextlib.closing(sqlite3.connect(self.dbfile)) as con, con as cur:
            cur.execute(
                "INSERT OR REPLACE INTO part_details_cache (lcsc, stock, type, description) "
                "VALUES (:lcsc, :stock, :type, :description)",
                {"lcsc": lcsc, "stock": stock, "type": type, "description": description}
            )
            con.commit()

    def get_part_details_cache(self, lcsc: str) -> dict:
        """Get cached part details by LCSC number."""
        with contextlib.closing(sqlite3.connect(self.dbfile)) as con, con as cur:
            con.row_factory = dict_factory
            return cur.execute(
                "SELECT * FROM part_details_cache WHERE lcsc = :lcsc",
                {"lcsc": lcsc}
            ).fetchone()
    ...
    def update_part(self, part: dict):
        """Update a part in the database, overwrite lcsc if supplied, preserve stock."""
        with contextlib.closing(sqlite3.connect(self.dbfile)) as con, con as cur:
            cur.execute(
            ...
            # Note: stock is intentionally NOT updated here to preserve persisted values
            cur.commit()
    ...
    def sync_stock_from_cache(self):
        """Backfill part_info.stock from part_details_cache for every part whose stock is empty."""
        with contextlib.closing(sqlite3.connect(self.dbfile)) as con, con as cur:
            try:
                cur.execute(
                    """UPDATE part_info SET stock = (
                        SELECT pdc.stock FROM part_details_cache pdc
                        WHERE pdc.lcsc = part_info.lcsc
                    )
                    WHERE (part_info.stock IS NULL OR part_info.stock = '')
                    AND part_info.lcsc != ''
                    AND EXISTS (
                        SELECT 1 FROM part_details_cache pdc WHERE pdc.lcsc = part_info.lcsc
                    )"""
                )
                cur.commit()
            except Exception:
                # part_details_cache may not exist in very old project databases
                pass
