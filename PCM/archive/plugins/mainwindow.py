...
ID_SETTINGS = 1005
ID_SELECT_PART = 1006
ID_REMOVE_LCSC_NUMBER = 1007
ID_SELECT_ALIKE = 1008
ID_TOGGLE_BOM_POS = 1009
ID_TOGGLE_BOM = 1010
ID_TOGGLE_POS = 1011
ID_PART_DETAILS = 1012
ID_HIDE_BOM = 1013
ID_HIDE_POS = 1014
ID_SAVE_MAPPINGS = 1015
ID_EXPORT_TO_SCHEMATIC = 1016
...
class JLCPCBTools(wx.Dialog):
    ...
    def populate_footprint_list(self, *_):
        ...
        for part in self.store.read_all():
            ...
            item_data = [
                ...
                part["footprint"],
                part["lcsc"],
                # Prefer type from cache
                str(details.get(part["lcsc"], {}).get("type", "") or ""),
                # Prefer stock from cache, fall back to native SQLite part_info storage; always str for DataViewCtrl
                str(details.get(part["lcsc"], {}).get("stock", "") or part.get("stock", "") or ""),
                part["exclude_from_bom"],
                part["exclude_from_pos"],
                str(self.get_correction(part, corrections)),
                str(fp.GetLayer()) if fp else "0",
                # Prefer raw description from cache; fall back to params_for_part for offline DB
                details.get(part["lcsc"], {}).get("description", "") or params_for_part(details.get(part["lcsc"], {})),
            ]
        ...
        if items_to_select:
            def apply_selection():
                ...
                self.enable_part_specific_toolbar_buttons(True)
            wx.CallAfter(apply_selection)
    ...
    def get_part_details(self, *_):
        """Fetch part details from LCSC and show them in a modal (one per unique LCSC number)."""
        seen = set()
        for item in self.footprint_list.GetSelections():
        ...
        for drawing in self.pcbnew.GetBoard().GetDrawings():
            if drawing.IsOnLayer(kicad_pcbnew.F_SilkS) or drawing.IsOnLayer(
                ...
                if (
                    ...
                    try:
                        if hasattr(drawing, "IsFilled") and not drawing.IsFilled():
                            continue
                    except Exception:
                        pass
    ...
    def generate_fabrication_data(self, *_):
        ...
        try:
            ...
            if number:
                ...
                layer_count = None
            self.logger.info("Generating Gerber files...")
            self.fabrication.generate_geber(layer_count)
            self.logger.info("Generating Excellon files...")
            self.fabrication.generate_excellon()
            self.logger.info("Zipping Gerber and Excellon files...")
            self.fabrication.zip_gerber_excellon()
            self.logger.info("Generating CPL file...")
            self.fabrication.generate_cpl()
            self.logger.info("Generating BOM file...")
        ...
        if success:
            if (lcsc := self.sanitize_lcsc(text_data.GetText())) != "":
                for item in self.footprint_list.GetSelections():
                    ...
                    params = details.get("description", params_for_part(details))
