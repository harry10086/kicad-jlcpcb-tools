...
class Library:
    ...
    def check_library(self):
        ...
        if (
            ...
            if use_szlcsc_online:
                self.logger.info("Using online SZLCSC search. Parts DB missing but update not required.")
    ...
    def search(self, parameters, page=1):
        """Search the database for parts that meet the given parameters."""
        use_szlcsc_online = self.parent.settings.get("general", {}).get("szlcsc_online_search", True)
        if use_szlcsc_online:
            return self.online_search(parameters, page)
    ...
    def online_search(self, parameters, page=1):
    ...
    def get_part_details(self, number: str) -> dict:
        """Get the part details for a LCSC number."""
        use_szlcsc_online = self.parent.settings.get("general", {}).get("szlcsc_online_search", True)
        if use_szlcsc_online:
            # First check the local project store cache if available
            store = getattr(self.parent, "store", None)
            if store:
                cached = store.get_part_details_cache(number)
                if cached:
                    return {
                        "lcsc": number,
                        "stock": cached["stock"],
            ...
            if os.path.isfile(self.partsdb_file) and os.path.getsize(self.partsdb_file) > 0:
                with contextlib.closing(sqlite3.connect(self.partsdb_file)) as con:
                    ...
                    part_info = next((n for n in cur.fetchall() if n["lcsc"] == number), {})

            if not part_info:
                # Provide a basic dict so it doesn't fail
                part_info = {"lcsc": number, "stock": "", "type": "", "part_no": "", "description": "", "package": "", "category": ""}
            return part_info
