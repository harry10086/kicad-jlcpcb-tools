...
class PartSelectorDataModel(dv.PyDataViewModel):
    ...
    def get_params(self, item):
        """Get the params of an item."""
        return self.ItemToObject(item)[self.columns["PARAMS_COL"]]

    def get_url(self, item):
        """Get the URL of an item."""
        row = self.ItemToObject(item)
        if len(row) > self.columns["URL_COL"]:
            return row[self.columns["URL_COL"]]
        return ""

    def get_datasheet(self, item):
        """Get the datasheet of an item."""
        row = self.ItemToObject(item)
        if len(row) > self.columns["DATASHEET_COL"]:
            return row[self.columns["DATASHEET_COL"]]
        return ""
